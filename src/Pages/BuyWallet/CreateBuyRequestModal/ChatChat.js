...
const ChatChat = ({ open, onClose, dataSingle, datao }) => {
  ...
  // const socket = io("https://api.redonion.io/api");
  ...
  var user = JSON.parse(localStorage.getItem("user"));
   ...
   const USER_CHAT_URL = "/chat/initiate-session";
    ...
    useEffect(() => {
      setMessageHistory([]);
      // messageHistoryApi(0, 10, props?.individual?.u_id).then(
      //   (response) => {
  ...
  //     .then((res) => {
  //       // console.log(res);
  //       setChatSession(res.data.data.chatSession);
  //     })
  //     .catch((err) => {
  ...
  return (
      ...
      <Box className={!isMobile ? styles.modalStyleA : styles.modalStyleMobile}>
        <Snackbar
