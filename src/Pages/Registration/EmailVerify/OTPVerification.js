...
import { Button, Stack, Typography, useMediaQuery, Snackbar, Alert, IconButton } from "@mui/material";
...
const OTPVerification = () => {
  ...
  const resendOTP = () => {
    setLoading(true);
    const email = Object.keys(allAuthContext.user).length
      > 0 ? allAuthContext.user.user.email : '';
    axios.post(
      RESEND_OPT_URL,
      JSON.stringify({
        "email": email,
